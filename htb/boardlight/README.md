...
![Dolibarr 17.0.0 login page](./images/Pasted_image_20260912173114.png)

Tested the default credential pair `admin:admin`, which succeeded and granted administrative access.

![Successful admin login, though showing an "access denied" message for an incomplete setup page](./images/Pasted_image_20260912173143.png)

**Findings:** The Dolibarr installation accepts default credentials (`admin:admin`) for its administrator account, with no forced password change or account lockout observed.
...
```
Websites -> + -> Name it -> Submit -> + (Page) -> Name (shell) -> Create
```
...
![Editing the page's HTML source with the injected payload](./images/Pasted_image_20260912174452.png)
...
![Editing the page source with the reverse shell payload](./images/Pasted_image_20260912175626.png)
...
![conf.php revealing the Dolibarr database username and plaintext password](./images/Pasted_image_20260912175900.png)
...
![/etc/passwd showing a local user "larissa" with a valid shell](./images/Pasted_image_20260912180105.png)
...
![Successful SSH login as larissa and capture of user.txt](./images/Pasted_image_20260912181140.png)
...
![LinPEAS output flagging several SUID binaries, including enlightenment_sys](./images/Pasted_image_20260912181259.png)

**Findings:** Among the flagged SUID binaries, `enlightenment_sys` and related Enlightenment utilities stood out as running with the SUID bit set (owned by root), with LinPEAS specifically flagging them as vulnerable to CVE-2022-37706 for versions before 0.25.4.
...
*(Version confirmed as 0.23.1, within the vulnerable range.)*
